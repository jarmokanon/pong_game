  ...
  rasterize(value) {
    ...
    for (let i = 0; i < string.length; i++) {
      ...
      for (let y = 0; y < height; y++) {
        ...
        array[y] = array[y].concat(
          !isLastElement ? char[y].concat([0]) : char[y]
        );
      }
    }
    return array;
  ...
  clear(index = null) {
    if (index !== null) {
      ...
      for (let i = 0; i < index; i++)
        x += this.charWidth[i] * this.game.blocksize + this.game.blocksize;
      this.game.hud.context.clearRect(
        this.left + this.align + x,
        this.top,
        width,
        this.size.y
      );
  ...
  draw() {
    ...
    for (let y = 0; y < height; y++) {
      for (let x = 0; x < width; x++) {
        if (this.pixels[y][x]) {
          this.game.hud.context.fillRect(
            this.left + this.align + x * bl,
            this.top + y * bl,
            bl,
            bl
          );
