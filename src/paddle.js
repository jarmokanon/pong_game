...
export default class Paddle extends Rectangle {
  ...
  update(deltatime) {
    if (this.top + this.velocity.y * deltatime < this.game.blocksize) {
      ...
      this.bottom + this.velocity.y * deltatime >
      this.game.canvas.height - this.game.blocksize
    ) {
      this.position.y =
        this.game.canvas.height - this.size.y / 2 - this.game.blocksize;
      this.velocity.y = 0;
    }
  ...
  reset() {
    this.position.x =
      this.index === 1
        ? this.size.x / 2 + this.game.blocksize
        : this.game.canvas.width - this.size.x / 2 - this.game.blocksize;
